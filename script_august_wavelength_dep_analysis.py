...
#               "BestDataFromWavelengthDependence_TRKR_vsV_300mT_" +
...
#               "lookingforDNP")
    ...
    def update_scandata_info(scandata):
        ...
        if 'BExternal' in scandata.info.keys():  # old style, convert T -> mT
            scandata.info['Magnetic Field (mT)'] = \
                                            1000 * scandata.info['BExternal']
...
#                             "Voltage (V)",
    ...
    def update_scandataset_model(scandataset):
        ...
        # SET MODEL INITIAL OSC_PERIOD
        set_bfield = \
            scandataset.scandata_list[0].info.get('Magnetic Field (mT)')
        if set_bfield is not None:
            osc_period_config = {'free': True,
                                 'initial value': 160000/set_bfield}
        set_model_params['osc_period'] = osc_period_config
...
#    params = model.model_params  # simplify handle
...
#            title_items.append(filename.split('_')[3])
    ...
    #        axes.set_title(title_str)
            ...
            if not figure_dict['ignore_fit']:
                ...
                if 'osc_period1' in fitdata.fitparamlabels:
                    param_ind1 = list(fitdata.fitparamlabels).index('osc_period1')
                    param_ind2 = list(fitdata.fitparamlabels).index('osc_period2')
                    gfactor_str = "osc_period1: {:.0f}".format(fitparams[param_ind1])
                    gfactor_str += " +- {:.2f}".format(fitparamstds[param_ind1])
                    gfactor_str += "\nosc_period2: {:.0f}".format(fitparams[param_ind2])
                    gfactor_str += " +- {:.2f}".format(fitparamstds[param_ind2])
...
if False:
# %%
    target_scandataset_ind = 0
    target_scandata_in_set_ind_list = [24]

    scandataset = scandataset_list[target_scandataset_ind]
    scandata_list_to_plot = [scandataset.scandata_list[ind]
                             for ind in target_scandata_in_set_ind_list]
    free_param_indices = \
        np.argwhere(scandataset.model.get_fit_params_is_free_param())
    param_labels = np.array(scandataset.model.get_fit_params_labels())
    free_param_labels = list(param_labels[free_param_indices])

    titlestr = "818.9nm, 300 mT, 4 V/cm"
    num_samples = 200
    param1ind = free_param_labels.index('osc_period1')
    param2ind = free_param_labels.index('osc_period2')
